package vn.vhn.virtualmouse

import android.Manifest.permission.SYSTEM_ALERT_WINDOW
import android.annotation.SuppressLint
import android.app.Activity
import android.content.Context
import android.content.Intent
import android.content.pm.PackageManager
import android.graphics.Color
import android.graphics.PixelFormat
import android.os.Build
import android.provider.Settings
import android.util.Log
import android.view.*
import androidx.constraintlayout.widget.ConstraintLayout
import androidx.constraintlayout.widget.ConstraintSet
import androidx.core.app.ActivityCompat
import androidx.core.content.ContextCompat
import androidx.core.content.ContextCompat.startActivity
import vn.vhn.vhscode.R


class VirtualMouse(val mActivity: Activity) {
    companion object {
        val TAG = "VirtualMouse"
        val TAP_THRESHOLD_MS = 100
        val INVALID_POINTER_ID = -1337
        val FAKE_TOUCH_SOURCE = 1337113377
        public fun fakeMotionEvent(motionEvent: MotionEvent): MotionEvent {
//            motionEvent.source = FAKE_TOUCH_SOURCE
            motionEvent.source = 1
            return motionEvent
        }

    }

    private var mRootView: View? = null
    private var mMouseView: MouseView? = null

    fun enable() {
        mRootView = mActivity.findViewById(R.id.root_container)
        val rootView = mRootView as? ConstraintLayout ?: return
        if (mMouseView != null && mMouseView!!.parent == rootView) {
            return
        }
        disable()
        mMouseView = MouseView(mActivity)
        mMouseView!!.id = View.generateViewId()
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            mMouseView!!.focusable = View.NOT_FOCUSABLE
        }
        val params = ConstraintLayout.LayoutParams(
            ConstraintLayout.LayoutParams.MATCH_PARENT,
            ConstraintLayout.LayoutParams.MATCH_PARENT
        )
        rootView.addView(mMouseView, params)
        val cons = ConstraintSet()
        cons.clone(rootView)
        for (dir in listOf(
            ConstraintSet.LEFT,
            ConstraintSet.TOP,
            ConstraintSet.RIGHT,
            ConstraintSet.BOTTOM
        ))
            cons.connect(mMouseView!!.id, dir, ConstraintSet.PARENT_ID, dir, 0)
        cons.applyTo(rootView)
    }

    fun disable() {
        if (mMouseView != null) {
            val parent = mMouseView?.parent as? ViewGroup
            parent?.removeView(mMouseView)
            mMouseView = null
        }
    }

    public fun moveMouseTo(x: Float, y: Float) {
        mMouseView?.update(x, y)
        mMouseView?.postInvalidate()
    }

}